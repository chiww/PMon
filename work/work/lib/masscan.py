...
__all__ = [
    'MasscanProcess', 'MasscanParser'
]


class MasscanParser(object):
    """
    当前仅解析端口扫描结果(不加--banner参数),否则其他扫描参数，可能会影响最终输出;
    """

    @classmethod
    def parse_from_file(cls, filepath, data_type='json'):

        with open(filepath, 'r') as parser_file:
            if data_type == 'json':
                return cls.parse_json(json.loads(parser_file.read()))

    @classmethod
    def parse_from_string(cls, stdout, data_type='json'):

        if data_type == 'json':
            return cls.parse_json(json.loads(stdout))

    @classmethod
    def parse_json(cls, stdout, to_list=True):

        kv_output = dict()  # {'10.0.0.97': {'ports': [], 'timestamp': 0 }}

        for host in stdout:
            if host['ip'] in kv_output.keys():
                kv_output[host['ip']]['ports'].append(host['ports'])
                kv_output[host['ip']]['timestamp'] = host['timestamp']
            else:
                kv_output[host['ip']] = {'ports': host['ports'], 'timestamp': host['timestamp']}

        if to_list:
            return cls.kv_to_list(kv_output)
        else:
            return kv_output

    @classmethod
    def kv_to_list(cls, kv_output):

        output = list()
        for address, info in kv_output.items():
            o = info
            o['address'] = address
            output.append(o)
...
class MasscanProcess(Thread):
    ...
    def sudo_run_background(self, run_as='root'):
        ...
        self.__sudo_run = "{0} -u {1}".format(sudo_path, sudo_user)
        super(MasscanProcess, self).start()
    ...
    def run(self):
        ...
        while self.__nmap_proc.poll() is None:
            for runstatus in iter(self.__nmap_proc.stderr.readline, ''):
                evnt = self.__process_event(runstatus)
    ...
    def __process_event(self, eventdata):
        ...
        """
        rval = False
        __default_taskname = 'SYN Scan'

        try:
            # --- nmaprun ---
            # 'Starting masscan 1.0.6 (http://bit.ly/14GZzcT) at 2020-01-04 08:53:31 GMT\n'
            if eventdata.startswith('Starting masscan'):
                m = re.match(r'Starting\smasscan\s([\d.]{5}) \(.+?\)\sat\s([\d\-\s:]{19}) GMT\n', eventdata)
                self.__starttime = int(time.time())
                # self.__starttime = m.group(2)
                self.__version = m.group(1)
                rval = True

            # ' -- forced options: -sS -Pn -n --randomize-hosts -v --send-eth\n'
            elif eventdata.startswith(' -- forced options'):
                pass

            # -- taskbgein ---
            # 'Initiating SYN Stealth Scan\n'
            elif eventdata.startswith('Initiating SYN Stealth Scan'):
                taskname = __default_taskname
            ...
            elif eventdata.startswith('Scanning'):
                m = re.match(r'Scanning\s(\d+)\shosts\s\[(\d+)\sports/host\]\n', eventdata)
                self.__nmap_tasks[__default_taskname].extrainfo = "hosts: %s, ports_per_host: %s" % (
                    m.group(1), m.group(2))

            # --- taskprogress ---
            # 'rate:  0.01-kpps,  2.08% done,   0:05:01 remaining, found=0       \n'
            elif eventdata.startswith('rate:') and 'remaining' in eventdata:
                m = re.match(r'rate:\s+([\d.]+)-kpps,\s+([\d.]+)%\s+done,\s*([\d:]+)\sremaining,\sfound=([\d]+)\s+\n',
                             eventdata)
                __rate = m.group(1)
                __percent = m.group(2)
                __remaining = m.group(3)    # 剩余时间
                __found = m.group(4)
                __etc = int(time.time())
                self.__nmap_tasks[__default_taskname].percent = __percent
                self.__nmap_tasks[__default_taskname].progress = __percent
                self.__nmap_tasks[__default_taskname].etc = __etc
                self.__nmap_tasks[__default_taskname].remaining = 0
                self.__nmap_tasks[__default_taskname].updated = __etc
                rval = True

            # --- taskend ---
            # 'rate:  0.00-kpps, 100.00% done, waiting 9-secs, found=5       \n'
            elif eventdata.startswith('rate:') and 'waiting' in eventdata:
                m = re.match(r'rate:\s+([\d.]+)-kpps,\s+([\d.]+)%\s+done,\swaiting\s([\d]+)-secs,\sfound=([\d]+)\s+\n',
                             eventdata)
                __rate = m.group(1)
                __percent = m.group(2)
                __waiting = m.group(3)
                ...
                self.__nmap_tasks[__default_taskname].percent = __percent
                self.__nmap_tasks[__default_taskname].progress = __percent
                self.__nmap_tasks[__default_taskname].status = "ended"
                rval = True

            # --- finish ---
            # '                                                                             \n'
            elif not eventdata.strip():
            ...
            print("Error [%s] >> [%s]" % (eventdata, str(e)))
            pass
    ...
    def mycallback(nmapscan=None):
        if nmapscan.is_running() and nmapscan.current_task:
            ...
            print("Task {0} ({1}): ETC: {2} DONE: {3}%".format(ntask.name,
                                                               ...
                                                               ntask.etc,
                                                               ntask.progress))
    nm = MasscanProcess("scanme.nmap.org",
                     options="-A",
